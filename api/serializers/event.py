...
class EventSchema(Schema):
    ...
    is_bookmarked = fields.Function(lambda event: event.is_bookmarked_by(Authenticator.get_instance().get_auth_user())
                                    if Authenticator.get_instance().get_auth_user() else False)
...
class EventReviewSchema(Schema):
    ...
    is_upvoted = fields.Function(lambda obj: obj.is_upvoted_by(Authenticator.get_instance().get_auth_user())
                                 if Authenticator.get_instance().get_auth_user() else False, required=True)
    is_downvoted = fields.Function(lambda obj: obj.is_downvoted_by(Authenticator.get_instance().get_auth_user())
                                   if Authenticator.get_instance().get_auth_user() else False, required=True)
